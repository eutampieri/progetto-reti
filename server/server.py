...
def handle_client(player):
    ...
    turn = 0
    while state == IN_GAME:
        player.msg(msgs["choose"](("Choose a question:", [
                   (str(i), "Question " + chr(ord('A') + i)) for i in range(3)])))
        ...
        player.msg(msgs["choose"]((question[0], [(str(i[0]), i[1])
                   for i in enumerate(question[1])])))
        ...
        if ans == question[2]:
            player.msg(msgs["message"](
                "Your answer was correct! You get a point"))
            player.score += 1
        else:
            player.msg(msgs["message"](
                "Your answer was wrong! You lose a point"))
            player.score -= 1
        turn += 1
    player.close()
